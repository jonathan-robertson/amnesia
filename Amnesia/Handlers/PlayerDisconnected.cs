...
namespace Amnesia.Handlers {
    internal class PlayerDisconnected {
        ...
        public static void Handle(ClientInfo clientInfo, bool forShutdown) {
            try {
                if (!API.ResetAfterDisconnectMap.ContainsKey(clientInfo.entityId)) {
                    ...
                    ResetDuringDisconnect(clientInfo);
        ...
        private static void ResetDuringDisconnect(ClientInfo clientInfo) {
            ...
            if (Config.RemoveSharedQuests) {
                ...
                clientInfo.latestPlayerData.questJournal.RemoveAllSharedQuests();
            } else {
            ...
            if (Config.ResetFactionPoints) {
                ...
                clientInfo.latestPlayerData.questJournal.QuestFactionPoints.Clear();
            } else {
    ...
    2022-07-07T18:13:55 1233.207 INF [Amnesia.API] [after] quests for Kanaverum
        ...
        public static void ResetQuests(ClientInfo clientInfo) { // TODO: make private
            ...
            clientInfo.latestPlayerData.questJournal.quests
                ...
                .ToList().ForEach(quest => {
                    try {
                        ...
                        log.Trace($"attempt: RemoveQuest {quest.ID}, code: {quest.QuestCode}, sharedOwnerCode: {quest.SharedOwnerID}, OwnerJournal.OwnerPlayer == null: {quest.OwnerJournal.OwnerPlayer == null}, player: {clientInfo.entityId}");
                        clientInfo.latestPlayerData.questJournal.RemoveQuest(quest);
                        log.Trace($"success: RemoveQuest {quest.ID}, code: {quest.QuestCode}, sharedOwnerCode: {quest.SharedOwnerID}, OwnerJournal.OwnerPlayer == null: {quest.OwnerJournal.OwnerPlayer == null}, player: {clientInfo.entityId}");
                    } catch (Exception e) {
                        log.Error($"failure: RemoveQuest {quest.ID}, code: {quest.QuestCode}, sharedOwnerCode: {quest.SharedOwnerID}, OwnerJournal.OwnerPlayer == null: {quest.OwnerJournal.OwnerPlayer == null}, player: {clientInfo.entityId}", e);
                    }
                });
            ...
            if (Config.ClearIntroQuests && QuestClass.s_Quests.ContainsKey("quest_BasicSurvival1")) {
                Quest quest = QuestClass.CreateQuest("quest_BasicSurvival1");
                if (quest != null) {
                    clientInfo.latestPlayerData.questJournal.AddQuest(quest);
                    log.Debug($"Added back quest_BasicSurvival1 quest to {clientInfo.entityId}");
                } else {
                    log.Debug($"Unable to find quest_BasicSurvival1; could not add quest to {clientInfo.entityId}");
                }
            }
        }
